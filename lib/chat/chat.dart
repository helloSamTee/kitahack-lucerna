  ...
  Future<String> _getGeminiResponse(String userMessage) async {
    ...
    try {
      final response = await http.get(
        Uri.parse('https://rag-api-463242189056.us-central1.run.app/query?query=${Uri.encodeComponent(prompt)}'),
      );

      if (response.statusCode == 200) {
        final responseData = jsonDecode(response.body);
        final responseText = responseData['response'] ?? 'No response from Gemini.';
...
Future<int> _setLLMKey(String apiKey) async {
  final apiUrl = 'https://rag-api-463242189056.us-central1.run.app/secure-endpoint/api_key'; // Replace with your API endpoint
